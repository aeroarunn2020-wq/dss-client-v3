...
class Client:
    def __init__(self, host='127.0.0.1', port=8625, authkey=b'769ac424-adb6-5a73-83b0-d22eb27e543b'):
        ...
        self.hashrate = 0.0
        self.last_event = "Ready to connect (KawPow)"
    ...
    def connect(self):
        ...
        self.manager.register('get_queue'); self.manager.register('result_queue')
    ...
    def mine(self, job_data: dict):
        ...
        header_hash = binascii.unhexlify(job.blob)
        target = job.target
        
        nonce = 0
        while self.connected:
            nonce_bytes = nonce.to_bytes(8, 'big') # KawPow uses 8-byte big-endian nonce
            hash_output, mix_hash = kawpow.kawpow_hash(header_hash, job.height, nonce)
    ...
    def run(self):
        ...
        while self.connected:
            try:
                ...
                if 'kawpow' not in job_algo:
                    self.last_event = f"Ignoring {job_algo.upper()} job (KawPow client)"
                    continue
                
                self.last_event = f"Starting new KawPow job: {job_data['job_id'][:8]}..."
