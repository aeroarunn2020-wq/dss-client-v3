...
class Client:
    def __init__(self, host='127.0.0.1', port=8625, authkey=b'769ac424-adb6-5a73-83b0-d22eb27e543b'):
        ...
        self.hashrate = 0.0
        self.last_event = "Ready to connect (RandomX)"
    ...
    def run(self):
        ...
        while self.connected:
            try:
                ...
                if 'rx' not in job_algo:
                    self.last_event = f"Ignoring {job_algo.upper()} job (RandomX client)"
                    continue
                
                self.last_event = f"Starting new RandomX job: {job_data['job_id'][:8]}..."
